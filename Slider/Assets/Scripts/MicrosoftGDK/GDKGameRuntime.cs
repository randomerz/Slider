...
{
    public static string GameConfigSandbox { get; private set; } = "DSKFNB.0";

    // Documented as: "Specifies the SCID to be used for Save Game Storage."
    public static string GameConfigScid { get; private set; } = GdkPlatformSettings.gameConfigScid; // "00000000-0000-0000-0000-0000FFFFFFFF";
    ...
    public static string GameConfigTitleId { get; private set; } = GdkPlatformSettings.gameConfigTitleId; // "FFFFFFFF";
    ...
    public static bool TryInitialize()
    {
        if (Instance == null)
        {
            Debug.Log("[GDK] GDK XGameRuntime Library not initialized.");
        ...
        Debug.Log($"[GDK] GDK XGameRuntime Library initialized: {Initialized}");
        Debug.Log($"[GDK] GDK Xbox Live API SCID: {GameConfigScid}");
    ...
    private void OnApplicationQuit()
    {
        ...
        Debug.Log("[GDK] Uninitializing Xbox Live API.");
        SDK.XBL.XblCleanup(null);
        Debug.Log("[GDK] Uninitializing XGame Runtime Library.");
    ...
    private static bool InitializeOrDestroyInstance(GDKGameRuntime newInstance)
    {
        ...
        {
            Debug.LogWarning($"[GDK] An instance of {newInstance.GetType().Name} already exist. Destroying {newInstance}...");
    ...
    private static bool InitializeRuntime(bool forceInitialization = false)
    {
        ...
        {
            Debug.Log($"[GDK] FAILED: Could not get TitleID! hResult: 0x{hResult:x} ({HR.NameOf(hResult)})");
        }
        ...
        if (titleId.ToString("X").ToLower().Equals(GameConfigTitleId.ToLower()) == false)
        {
            ...
            Debug.LogWarning($"WARNING! Also Expected Title Id: {GameConfigTitleId:X}");
        }
        ...
        if (HR.FAILED(hResult))
        {
            Debug.Log($"[GDK] FAILED: Could not get SandboxID! HResult: 0x{hResult:x} ({HR.NameOf(hResult)})");
        }
        ...
        Debug.Log($"[GDK] GDK Initialized, titleId: {GameConfigTitleId}, sandboxId: {GameConfigSandbox}");
    ...
    private static int InitializeGamingRuntime(bool forceInitialization = false)
    {
        ...
        Debug.Log("[GDK] Initializing XGame Runtime Library.");

        if (Initialized && forceInitialization == false)
        {
            Debug.Log("[GDK] Gaming Runtime already initialized.");
            return 0;
        }
        ...
        if (HR.FAILED(hResult))
        {
            Debug.Log($"[GDK] FAILED: Initialize XGameRuntime, HResult: 0x{hResult:X} ({HR.NameOf(hResult)})");
            return hResult;
        }
    ...
    {
        Debug.Log($"[GDK] Initializing Xbox Live API (SCID: {scid}).");

        int hResult = SDK.XBL.XblInitialize(scid);
        if (HR.FAILED(hResult) && hResult != HR.E_XBL_ALREADY_INITIALIZED)
        {
            Debug.Log($"[GDK] FAILED: Initialize Xbox Live, HResult: 0x{hResult:X}, {HR.NameOf(hResult)}");
            return false;
        }
