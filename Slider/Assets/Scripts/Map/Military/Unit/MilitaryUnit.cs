...
public class MilitaryUnit : MonoBehaviour
{
    ...
    public void DoDeathAnimation(System.Action onAnimationResolved)
    {
        ...
        CoroutineUtils.ExecuteAfterDelay(
            ...
            this, 
            MilitaryTurnAnimator.CurrentGlobalAnimationsSpeed == MilitaryTurnAnimator.Speed.Fast ? 0.06125f : 0.25f
        );
    }
    ...
    private void SpawnSliderReward()
    {
        ...
        {
            Debug.LogWarning($"Tried spawning a slider on an inactive STile! (loc: {GridPosition}, island id: {stile.islandId}). Spawning on player's instead.");
            stile = (MilitarySTile)SGrid.GetSTileUnderneath(Player.GetInstance().gameObject);
        }

        if (stile == null)
        {
            Debug.LogWarning($"Player wasn't standing on a tile either, spawning on stile 1 instead.");
            stile = (MilitarySTile)SGrid.Current.GetStile(1);
        }
    ...
    private void StartCombatWithOverlappingEnemyUnit()
    {
        ...
        activeUnits.ForEach(unit =>
        {
            if (
                ...
                unit.UnitTeam != UnitTeam &&
                unit.UnitStatus == Status.Active
            )
            {
